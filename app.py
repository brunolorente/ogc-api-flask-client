...
def get_features(collectionId):
    ...
    try:
        ...
        if (queriable != '' and queriable != None and queriable_value != '' and queriable_value != 'None'): 
            if (queriable == 'bbox'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, bbox=queriable_value.split(','))
            if (queriable == 'properties'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, property_name=queriable_value)
            if (queriable == 'crs'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, crs=queriable_value)
            if (queriable == 'lat'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, lat=queriable_value)
            if (queriable == 'lon'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, lon=queriable_value)
            if (queriable == 'radius'):
                api_response = api_instance.get_features(collection_id, f=f, limit=limit, pretty=pretty, radius=queriable_value)
        else:
