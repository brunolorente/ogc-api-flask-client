...
@app.route('/foundation/collections/<collectionId>/items')
@app.route('/foundation/collections/<collectionId>/items/')
def foundation_get_features(collectionId="aerofacp_1m"):
    # Get the collection ID
    collection_id = collectionId
    # Set the API resource url
    URL = "https://test.cubewerx.com/cubewerx/cubeserv/demo/ogcapi/Foundation/collections/"+collection_id+"/items"
...
@app.route('/foundation/collections/<collectionId>/items/<itemId>')
@app.route('/foundation/collections/<collectionId>/items/<itemId>/')
def foundation_get_feature(collectionId="aerofacp_1m",itemId="CWFID.AEROFACP_1M.0.0.1009C50A276905DE1F20020000"):
    # Get the collection ID
    collection_id = collectionId
    # get the item ID
    item_id = itemId
    # Set the API resource url
    URL = "https://test.cubewerx.com/cubewerx/cubeserv/demo/ogcapi/Foundation/collections/"+collection_id+"/items/"+item_id
    f = "json" # str | A MIME type indicating the representation of the resources to be presented (e.g. application/gml+xml; version=3.2 for GML 3.2). (optional)
    # defining a params dict for the parameters to be sent to the API
    PARAMS = {
        'f':f,
...
def get_features(collectionId):
    ...
    f = 'json' # str | A MIME type indicating the representation of the resources to be presented (e.g. application/gml+xml; version=3.2 for GML 3.2). (optional)
    limit = 1000000 # int | The optional limit parameter limits the number of items that are presented in the response document.  Only items are counted that are on the first level of the collection in the response document. Nested objects contained within the explicitly requested items shall not be counted. (optional) (default to 10)
